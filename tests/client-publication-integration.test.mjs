...
test('publicação reconhece e valida o Projeto de Evolução e o Contrato',()=>{
 ...
 assert.ok(api.includes('Preparar o Contrato ou Termo'));
});
...
test('painel apresenta checklist e próxima ação do fluxo integrado',()=>{
 for(const text of ['Projeto de Evolução preparado','Contrato preparado','Área pronta para publicação','Aguardando formalização','Iniciar implantação','Publicação e Acesso'])assert.ok(panel.includes(text),text);
