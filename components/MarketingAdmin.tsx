...
export function MarketingParameters(){
 ...
 const save=async()=>{setBusy(true);setError('');setMessage('');try{const params=new URLSearchParams(location.search),r=await fetch('/api/commercial',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'marketing',platforms:rows,empresa_id:params.get('empresa'),projeto_evolucao_id:params.get('projeto')})}),x=await r.json();if(!r.ok)throw new Error(x.error);setMessage(x.pendency_completed?'Configurações salvas e Pendência Inteligente concluída.':'Configurações operacionais de Marketing salvas com sucesso.');await load()}catch(e:any){setError(e?.message||'Não foi possível salvar Marketing.')}finally{setBusy(false)}};
