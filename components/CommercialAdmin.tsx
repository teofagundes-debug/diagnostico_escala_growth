...
export function CommercialParameters(){
 ...
 const remove=async(id:string)=>{if(confirm('Arquivar este produto/serviço?')){await fetch('/api/commercial?id='+id,{method:'DELETE'});load()}};
 const newResource=()=>setEditing({codigo:'',categoria:'',nome:'',descricao:'',tipo:'Implantação',ui:1,valor_mensal:'',valor_avulso:'',obrigatoriedade:'Opcional',responsavel:'Escala Vendas',prioridade:'Média',observacoes:'',ativo:true});
 ...
 {tab==='services'&&<><section className="admin-section"><div className="module-toolbar"><div><span className="eyebrow">Catálogo interno</span><h2>Produtos e Serviços</h2></div><button className="btn btn-primary" onClick={newResource}>Novo produto/serviço</button></div>
 <div className="resource-admin-list">{data.resources.map((x:any)=><article key={x.id} className={!x.ativo?'inactive':''}><b>{x.codigo}</b><span>{x.tipo}</span><strong>{x.nome}</strong><span>{x.tipo==='Implantação'?x.ui+' UI':money(x.tipo==='Mensalidade'?x.valor_mensal:x.valor_avulso)}</span><span>{x.obrigatoriedade||'Opcional'}</span><span>{x.responsavel}</span><button onClick={()=>setEditing(x)}>Editar</button><button onClick={()=>remove(x.id)}>Arquivar</button></article>)}</div></section>
 ...
 {editing&&<div className="modal"><section><h2>{editing.id?'Editar':'Novo'} produto/serviço</h2><div className="resource-form">
  ...
  <label>Obrigatoriedade<select value={editing.obrigatoriedade||'Opcional'} onChange={e=>setEditing({...editing,obrigatoriedade:e.target.value})}><option>Obrigatório</option><option>Opcional</option></select></label>
  ...
  <label>Nome<input value={editing.nome||''} onChange={e=>setEditing({...editing,nome:e.target.value})}/></label>
  ...
  {editing.tipo==='Avulso'&&<label>Valor<input type="number" min="0" step="0.01" value={editing.valor_avulso??''} onChange={e=>setEditing({...editing,valor_avulso:e.target.value})}/></label>}
  <label>Responsável<input value={editing.responsavel||''} onChange={e=>setEditing({...editing,responsavel:e.target.value})}/></label>
  <label>Prioridade<input value={editing.prioridade||''} onChange={e=>setEditing({...editing,prioridade:e.target.value})}/></label>
  <label>Descrição<textarea rows={3} value={editing.descricao||''} onChange={e=>setEditing({...editing,descricao:e.target.value})}/></label>
  <label>Observações<textarea rows={3} value={editing.observacoes||''} onChange={e=>setEditing({...editing,observacoes:e.target.value})}/></label>
 </div>{formError&&<p className="error">{formError}</p>}<div className="detail-actions"><button className="btn btn-secondary" onClick={()=>{setEditing(null);setFormError('')}}>Cancelar</button><button className="btn btn-primary" onClick={saveResource}>Salvar produto/serviço</button></div></section></div>}</>
}
