...
export function MeetingPreparation({data,onComplete}:{data:any;onComplete:()=>void}){
 ...
 useEffect(()=>{fetch('/api/meeting-preparation?id='+data.id,{cache:'no-store'}).then(async r=>{const x=await r.json();if(r.ok){setRecord(x);setReadiness(x.prontidao||{});setChecks(x.pontos_validacao||{});setValidations(x.validacoes_reuniao||{});if(x.status==='Em condução')setMode('conduct')}})},[data.id]);
 ...
 const conclude=async()=>{if(!meetingId){setMessage('Não foi possível localizar o vínculo da reunião. Volte à Agenda, abra novamente a Preparação da Reunião e tente concluir.');return}if(!confirm('Concluir a Reunião Estratégica e gerar a base oficial do Plano Estratégico?'))return;setBusy(true);const payload={...record,empresa_id:data.empresa_id,diagnostico_id:data.id,reuniao_id:meetingId,recomendacoes:recommendations,prontidao:effective,pontos_validacao:checks,validacoes_reuniao:validations,status:'Concluída'};delete payload.meeting;const r=await fetch('/api/meeting-preparation',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),response=await r.json().catch(()=>({}));setBusy(false);if(r.ok){setMessage('Reunião concluída. O Plano Estratégico está disponível para atualização.');onComplete()}else setMessage(response.error||'Não foi possível concluir a reunião. Tente novamente.')};
 ...
 return <section className="meeting-preparation admin-section">
  ...
  <div className="detail-actions"><button className="btn btn-secondary" disabled={busy} onClick={()=>save()}>{busy?'Salvando...':'💾 Salvar Preparação'}</button>{mode==='prepare'?<button className="btn btn-primary" onClick={start}>▶️ Iniciar Reunião Estratégica</button>:<button className="btn btn-primary" disabled={busy} onClick={conclude}>✅ Concluir Reunião</button>}</div>{message&&<p className={message.startsWith('Não')||message.startsWith('Agende')?'error':'success'}>{message}</p>}
 </section>
}
