...
export function portalProjectEvolutionProjection(input:{project:any;commercialSnapshot?:any;currentSituation?:any;officialPlan?:any;officialImplementation?:any;officialItems?:any[]}){
 ...
 project.mensalidade_atual=currentMonthly;project.nova_mensalidade=frozenTotal;
 return{project,financial:{current_monthly:currentMonthly,additional_monthly:Number(project.mensalidade_adicional||0),new_monthly_total:frozenTotal,current_source:'SITUACAO_COMERCIAL_VIGENTE',additional_source:'PROJETO_PUBLICADO',total_source:input.commercialSnapshot?.financial?'CONSOLIDACAO_COMERCIAL_3_0':'PROJETO_PUBLICADO'},official_execution:{available:official,plan_id:input.officialPlan?.id||null,plan_version:input.officialPlan?.version_number||null,implementation_id:input.officialImplementation?.id||null}};
}
