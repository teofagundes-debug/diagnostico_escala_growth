...
export function composeGrowthProject({catalog,activeResources=[],priority='Organizar',baseClient=false,signals={}}:{catalog:any[];activeResources?:string[];priority?:string;baseClient?:boolean;signals?:DecisionSignals}){
 ...
 let strategic=[...candidates.values()].filter(candidate=>!removed.some(name=>normalize(name)===normalize(candidate.name))).map(candidate=>{const item=find(catalog,candidate.name);return item?{...item,classificacao:(candidate.weight>=8?'Recomendado':'Opcional') as GrowthClassification,origem:candidate.reasons.join(' · '),motivos_decisao:candidate.reasons,peso:candidate.weight,fase:candidate.weight>=8?'Recomendações Estratégicas':'Evoluções Futuras'}:null}).filter(Boolean).filter((item:any)=>!has(unavailable,item.nome));
