...
export function buildClientStrategicView(input:{plans:any[];implementations:any[];items:any[];measurements:any[];measurementItems:any[]}){
 ...
 const groupedActions=Object.fromEntries(['AGORA','DEPOIS','QUANDO_ESTIVER_PRONTO'].map(horizon=>[horizon,actions.filter((action:any)=>action.agreed_horizon===horizon).map((action:any)=>({...action,horizon_label:horizonLabels[horizon]}))]));
