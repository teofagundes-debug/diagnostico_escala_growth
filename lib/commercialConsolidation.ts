...
export const commercialSource=(project:any)=>({
 // Status and audit timestamps are excluded: publishing unchanged commercial
 // content must not make its own frozen consolidation stale.
 project:{id:project.id,nome:project.nome||null,tipo:project.tipo||null,descricao:project.descricao||null,objetivo:project.objetivo||null,valor_implantacao_adicional:Number(project.valor_implantacao_adicional||0),implantacao_modalidade:project.implantacao_modalidade||null,nova_mensalidade:Number(project.nova_mensalidade||0),desconto_recorrente:Number(project.desconto_recorrente||0),formalizacao:project.formalizacao||null},
