...
export async function POST(req:Request){try{
 ...
 if(body.action==='marketing'){const allowed=['Escala Vendas','Cliente','Parceiro'];for(const p of body.platforms||[]){const data={plataforma:p.plataforma,codigo:p.codigo,ui:Number(p.ui||0),valor_gestao_mensal:Number(p.valor_gestao_mensal||0),responsavel_padrao:allowed.includes(p.responsavel_padrao)?p.responsavel_padrao:'Cliente',investimentos:p.investimentos||{},objetivo_padrao:p.objetivo_padrao||null,ativo:p.ativo!==false,updated_at:new Date().toISOString()};await write('marketing_parametros?on_conflict=plataforma','POST',data)}return Response.json({ok:true})}
