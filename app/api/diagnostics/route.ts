...
function admin(req:Request){return !!ADMIN_KEY&&req.headers.get('authorization')===`Bearer ${ADMIN_KEY}`}
...
export async function GET(req:Request){
 if(!admin(req))return Response.json({error:'Não autorizado'},{status:401});
 if(!ready())return Response.json({error:'Persistência não configurada'},{status:503});
 ...
 const q=id?`id=eq.${encodeURIComponent(id)}&select=${select}`:`select=id,data_diagnostico,pontuacao_geral,nivel_maturidade,menor_pilar,potencial_crescimento,status,created_at,empresas(nome),responsaveis(nome)&order=created_at.desc`;
