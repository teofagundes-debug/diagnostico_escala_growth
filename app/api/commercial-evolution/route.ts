...
async function context(empresaId:string){
 const id=encodeURIComponent(empresaId),[rawVersions,rawProjects,rawPendencies,rawExecutionHistory,rawCatalog,rawDiagnostics,rawMeetings,rawPlans]=await Promise.all([
...
export async function PATCH(req:Request){
 try{
  ...
  if(body.action==='execution-strategy'){
   ...
   try{
    ...
    if(strategic){
     ...
     saveStage='SAVE_CANONICAL_FINANCIAL';await saveGrowthFinancial(db,{companyId:existing.empresa_id,projectEvolutionId:existing.id,payload:{valor_implantacao:frozenImplantation,valor_mensalidade:frozenMonthly,prazo_contratual:canonicalFinancial.prazo_contratual,validade_proposta:canonicalFinancial.validade_proposta,desconto_pix:canonicalFinancial.desconto_pix,updated_at:now}});
  ...
  if(body.action==='formalize'){
   ...
   if(existing.exige_pagamento){const financial=(await loadGrowthFinancial(db,{companyId:existing.empresa_id,projectEvolutionId:existing.id}))?.financial;if(financial?.status!=='Pagamento confirmado')return Response.json({error:'Confirme o pagamento antes da formalização deste projeto.'},{status:409});}
