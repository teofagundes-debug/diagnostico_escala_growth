...
const parametersSnapshot=(item:any)=>item.parametros_snapshot&&Object.keys(item.parametros_snapshot).length?item.parametros_snapshot:(item.parametros_metodo||{objetivo_padrao:item.objetivo_padrao||null,tipo_implantacao:item.tipo_implantacao||null,tempo_medio_implantacao:item.tempo_medio_implantacao||null,treinamento_obrigatorio:item.treinamento_obrigatorio===true,gera_pendencias:item.gera_pendencias===true,abre_planejamento_operacional:item.abre_planejamento_operacional===true,permite_executor_terceiro:item.permite_executor_terceiro!==false,permite_equipe_interna:item.permite_equipe_interna!==false,investimento_minimo_recomendado:amount(item.investimento_minimo_recomendado),investimento_ideal_minimo:item.investimento_ideal_minimo==null?null:amount(item.investimento_ideal_minimo),investimento_ideal_maximo:item.investimento_ideal_maximo==null?null:amount(item.investimento_ideal_maximo),observacoes_estrategicas:item.observacoes_estrategicas||null});
