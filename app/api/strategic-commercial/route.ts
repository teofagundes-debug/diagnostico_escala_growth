...
import {resolveCanonicalExistingResources} from '../../../lib/existingResourceResolver';
...
export async function GET(req:Request){try{
 ...
 const [plans,meetings,situations]=await Promise.all([db(`strategic_execution_plans?empresa_id=eq.${encodeURIComponent(empresaId)}&diagnostico_id=eq.${encodeURIComponent(diagnostic.id)}&select=id,empresa_id,diagnostico_id,version_number,status,strategic_direction,primary_priority,contextual_prescriptions,contextual_prescription_version&order=version_number.desc&limit=1`).catch(()=>[]),db(`reunioes_estrategicas?empresa_id=eq.${encodeURIComponent(empresaId)}&diagnostico_id=eq.${encodeURIComponent(diagnostic.id)}&select=id,dados_reuniao,created_at&order=created_at.desc`).catch(()=>[]),db(`situacoes_comerciais_versoes?empresa_id=eq.${encodeURIComponent(empresaId)}&vigente=eq.true&select=id,recursos,versao&limit=1`).catch(()=>[])]),plan=plans[0]||null;
 ...
 const reviewedMeeting=meetings.find((item:any)=>Array.isArray(item?.dados_reuniao?.diagnostico_validado?.recomendacoes_adaptativas)),adaptiveRecommendations=reviewedMeeting?.dados_reuniao?.diagnostico_validado?.recomendacoes_adaptativas||[],existingResources=resolveCanonicalExistingResources({catalog:data.catalog,commercialResources:situations[0]?.recursos,adaptiveRecommendations}),context={empresa_id:empresaId,diagnostic_id:diagnostic.id,plan_id:plan?.id||null,plan_version:plan?.version_number||null,plan_status:plan?.status||null};
 ...
 const composition=resolveStrategicSolutions({interventions,links:data.links,catalog:data.catalog,valor_ui:Number(data.parameters?.valor_ui||0),excluded_solution_ids:excludedSolutionIds,existing_resources:existingResources});
 return Response.json({flow:'ESTRATEGICO_3_0',...context,decision,active_interventions:interventions,contextual_prescriptions:contextualPrescriptions,context_source:contextualPrescriptions.length?'PLANO_EXECUTAVEL_REVISAO':'DIAGNOSTICO_ORIGINAL',existing_resources:existingResources.map(item=>({recurso_id:item.recurso_id,state:item.state,sources:item.sources,recurring:item.recurring})),composition,coverage,parameters:data.parameters,catalog:data.catalog});
