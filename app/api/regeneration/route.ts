...
export async function POST(req:Request){
 try{
  ...
  await db('regeneracoes_metodo',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({empresa_id:empresaId,diagnostico_id:ctx.diagnostic.id,plano_id:ctx.plan.id,tipo,versao,metodo_versao:METHOD_VERSION,versao_metodo_anterior:previousMethodVersion,versao_metodo_nova:METHOD_VERSION,motivo,usuario:body.usuario||'Usuário Master',opcoes:body,versao_anterior:previous,versao_nova:next,comparacao:comparison})});
