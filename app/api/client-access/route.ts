...
export async function POST(req:Request){
 ...
 }catch(e:any){return Response.json({error:e?.message||'Não foi possível concluir a Publicação.'},{status:500})}
}
export async function PATCH(req:Request){try{const body=await req.json(),token=String(body.access_token||'');if(!token||!SUPABASE_URL||!ANON)return Response.json({error:'Convite inválido.'},{status:400});const u=await fetch(`${SUPABASE_URL}/auth/v1/user`,{headers:{apikey:ANON,Authorization:`Bearer ${token}`}});if(!u.ok)return Response.json({error:'Convite inválido ou expirado.'},{status:401});const user=await u.json();const invitation=(await rest(`portal_usuarios?email=eq.${encodeURIComponent(String(user.email).toLowerCase())}&select=convite_expira_em&limit=1`))?.[0];if(invitation?.convite_expira_em&&new Date(invitation.convite_expira_em)<new Date())return Response.json({error:'Este convite expirou. Solicite um novo convite à Escala Vendas.'},{status:410});if(!body.password||String(body.password).length<8)return Response.json({error:'A senha deve possuir pelo menos 8 caracteres.'},{status:400});const changed=await fetch(`${SUPABASE_URL}/auth/v1/user`,{method:'PUT',headers:{apikey:ANON,Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({password:body.password})});if(!changed.ok)return Response.json({error:'Senha não aceita. Por favor, digite outra senha.'},{status:400});const now=new Date().toISOString();await rest(`portal_usuarios?email=eq.${encodeURIComponent(String(user.email).toLowerCase())}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({auth_user_id:user.id,status_acesso:'Acesso ativado',ativo:true,acesso_ativado_em:now,primeiro_acesso_em:now,ultimo_acesso_em:now,convite_link:null,updated_at:now})});const profile=(await rest(`portal_usuarios?email=eq.${encodeURIComponent(String(user.email).toLowerCase())}&select=empresa_id&limit=1`))?.[0];if(profile?.empresa_id)await audit(profile.empresa_id,undefined,'Primeiro acesso realizado','O cliente definiu sua senha e ativou o acesso.');return Response.json({ok:true})}catch(e:any){return Response.json({error:e?.message||'Não foi possível ativar o acesso.'},{status:500})}}
