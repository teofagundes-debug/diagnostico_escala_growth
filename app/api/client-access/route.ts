...
import {buildHomologatedContract} from '../../../lib/contractTemplate';
...
async function rest(path: string, init: RequestInit = {}) { const r = await fetch(`${SUPABASE_URL}/rest/v1/${path}`, { ...init, headers: { ...h(), ...(init.headers || {}) }, cache: 'no-store' }); if (!r.ok)
...
async function sendEmail(input: {
    ...
    existing?: boolean;
    tools?: boolean;
}) { const key = process.env.RESEND_API_KEY, from = process.env.EMAIL_FROM; if (!key || !from)
    return { sent: false, error: 'Serviço de e-mail ainda não configurado. Copie o link e envie manualmente.' }; const subject = input.existing ? 'Cadastro de senha | Área do Cliente Escala Vendas' : input.tools ? 'Sua proposta está disponível | Escala Vendas' : 'Seu acesso à Central Escala Growth está disponível'; const title = input.existing ? 'Cadastre uma nova senha para acessar sua área.' : input.tools ? 'Sua Proposta Comercial de Implantação de Ferramentas está disponível.' : 'Seu Plano Estratégico Escala Growth já está disponível.'; const description=input.tools?'Na Área do Cliente você poderá revisar a Proposta Comercial e o Contrato ou Termo, realizar o aceite e consultar seus documentos.':'Na Central você poderá acompanhar diagnóstico, planos, investimento, aceite, documentos, implantação e evolução do IEG.'; const button = input.existing ? 'CADASTRAR NOVA SENHA' : 'CRIAR MINHA SENHA E ACESSAR'; const html = `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#17231c"><h2>Olá, ${input.name}.</h2><p>${title}</p><p>${description}</p><p><a href="${input.link}" style="display:inline-block;background:#15824b;color:white;padding:14px 22px;border-radius:8px;text-decoration:none;font-weight:bold">${button}</a></p><hr><p><b>Escala Vendas</b><br>Soluções para organizar e evoluir sua operação comercial.</p></div>`; const r = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [input.email], subject, html }) }); return r.ok ? { sent: true } : { sent: false, error: (await r.text()).slice(0, 300) }; }
async function sendWelcomeEmail(input: {
    email: string;
    name: string;
    tools?: boolean;
}) { const key = process.env.RESEND_API_KEY, from = process.env.EMAIL_FROM; if (!key || !from)
    return; const portalUrl = APP + '/login', html = `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#17231c"><h2>Olá, ${input.name}.</h2><p>Seu acesso foi configurado com sucesso.</p><p>Sempre que desejar acessar sua área do cliente utilize:</p><p><a href="${portalUrl}" style="color:#15824b;font-weight:bold">${portalUrl}</a></p><p><b>Usuário:</b><br>${input.email}</p><p>${input.tools?'Na sua área você encontra a Proposta Comercial, o Contrato ou Termo, o aceite e os documentos da contratação.':'Na Central você poderá acompanhar sua jornada no Método Escala Growth.'}</p><p>Em caso de dúvidas, entre em contato com nossa equipe.</p><p><b>Equipe Escala Vendas</b></p></div>`; await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [input.email], subject: input.tools?'Bem-vindo à Área do Cliente Escala Vendas':'Bem-vindo à Escala Growth', html }) }); }
...
async function officialPublicationContext(empresaId: string) {
    ...
    if (publishedPlan) {
        implementation = (await rest(`strategic_plan_implementations?plan_id=eq.${encodeURIComponent(publishedPlan.id)}&plan_version=eq.${encodeURIComponent(publishedPlan.version_number)}&select=*&limit=1`))?.[0] || null;
        if (implementation)
            items = await rest(`strategic_plan_implementation_items?implementation_id=eq.${encodeURIComponent(implementation.id)}&select=*`);
    }
...
async function canonicalPublicationReadiness(context: any, financial: any) {
    ...
    if (context.legacyFallback)
        return { legacy: true, implementation, commercial: null, legacyState: legacyPublicationReadiness(context.project) };
    let fingerprintMatches = false;
    if (context.project?.commercial_3_0_snapshot) {
        const fingerprint = await commercialFingerprint(context.project);
        fingerprintMatches = Boolean(context.project.commercial_3_0_fingerprint && context.project.commercial_3_0_fingerprint === fingerprint);
    }
...
export async function GET(req: Request) {
    try {
        ...
        const [data, profiles, publications, diagnostics, plans, implementations, acceptances, payments, projects, contracts, intelligentPendencies] = await Promise.all([
        ...
        return Response.json({ error: e?.message || 'Não foi possível carregar a Publicação.' }, { status: 500 });
    }
}
export async function POST(req: Request) {
    try {
        ...
        if (['deactivate', 'reactivate'].includes(action)) {
            ...
            if (!existing)
                return Response.json({ error: 'Acesso ainda não criado.' }, { status: 404 });
        ...
        if (action === 'operational_stage') {
            ...
            if (!stage)
                return Response.json({ error: 'Etapa operacional inválida.' }, { status: 400 });
            ...
            if (body.stage === 'kickoff') {
                ...
                if (project) {
                    ...
                    for (const item of marketing) {
                        const name = String(item.nome_snapshot || ''), platform = /google/i.test(name) ? 'Google Ads' : /meta/i.test(name) ? 'Meta Ads' : /landing/i.test(name) ? 'Landing Page' : /whatsapp/i.test(name) ? 'Campanhas WhatsApp' : name;
                        await rest('planejamentos_campanhas?on_conflict=projeto_evolucao_id,recurso_id,plataforma', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify({ empresa_id: empresaId, projeto_evolucao_id: project.id, recurso_id: item.recurso_id, plataforma: platform, objetivo: item.parametros_snapshot?.objetivo_padrao || item.executor_dados?.objetivo || null, investimento_recomendado: Number(item.investimento_recomendado || 0), investimento_aprovado: Number(item.investimento_aprovado || 0), executor: item.executor || null, responsavel_configuracao: 'Equipe de Implantação', status: 'Planejamento', updated_at: now }) });
                    }
                    if (marketing.length) {
                        const existingPendency = (await rest(`pendencias_inteligentes?projeto_evolucao_id=eq.${encodeURIComponent(project.id)}&codigo=eq.MARKETING_PARAMETROS&select=id&limit=1`))?.[0], pendency = { empresa_id: empresaId, projeto_evolucao_id: project.id, codigo: 'MARKETING_PARAMETROS', titulo: 'Planejamento Operacional das Campanhas', categoria: 'Implantação', status: 'Pendente', rota_configuracao: '/central/parametros-marketing', updated_at: now };
                        await rest(existingPendency ? `pendencias_inteligentes?id=eq.${encodeURIComponent(existingPendency.id)}` : 'pendencias_inteligentes', { method: existingPendency ? 'PATCH' : 'POST', headers: { Prefer: 'return=minimal' }, body: JSON.stringify(pendency) });
                    }
                }
            }
        ...
        if (action === 'review_contract') {
            ...
            if (!contract)
                return Response.json({ error: 'O contrato ainda não foi iniciado.' }, { status: 409 });
            if (missing.length)
                return Response.json({ error: 'Complete os dados contratuais antes da revisão.', pending: missing }, { status: 409 });
        ...
        if(action==='update_publication') {
            ...
            const [financialContext, plan, implementation, project, contract, diagnostic, lastPublication] = await Promise.all([
                loadGrowthFinancial(rest, { companyId: empresaId, projectEvolutionId: projectId }),
            ...
            const financial = financialContext?.financial || null;
            if (!project || !lastPublication)
                return Response.json({ error: 'Não existe uma publicação anterior desta empresa para atualizar.' }, { status: 409 });
            if (!isStrategicCommercialProject(project) || project.commercial_3_0_status !== 'PRONTO' || !project.commercial_3_0_snapshot)
                return Response.json({ error: 'Conclua novamente a Consolidação Comercial 3.0 antes de atualizar o cliente.' }, { status: 409 });
            const currentFingerprint = await commercialFingerprint(project);
            if (project.commercial_3_0_fingerprint !== currentFingerprint)
                return Response.json({ error: 'A consolidação está desatualizada. Consolide novamente antes de atualizar o cliente.' }, { status: 409 });
            ...
            if (currentResources.length !== snapshotResources.length)
                return Response.json({ error: 'Os recursos do Projeto divergem do snapshot consolidado. Revise a Composição Comercial 3.0.' }, { status: 409 });
            if (!snapshotResources.length && publishedResources.length)
                return Response.json({ error: `Atualização bloqueada: a versão publicada possui ${publishedResources.length} recursos e o novo snapshot está vazio. Revise os vínculos canônicos antes de continuar.`, code: 'EMPTY_RESOURCE_REGRESSION' }, { status: 409 });
        ...
        if(action==='publish') {
            ...
            if(tools&&!ctx.diagnosticoId){
                ...
                const [providerRows,parameterRows]=await Promise.all([rest('configuracoes?select=*&limit=1').catch(()=>[]),rest('parametros_comerciais?select=*&limit=1').catch(()=>[])]),contractPayload={empresa_id:empresaId,formalizacao_id:formalization.id,titulo:'Contrato/Termo de Implantação de Ferramentas',status:'Publicado',versao:String(proposalVersion),conteudo:buildHomologatedContract({origin:'IMPLANTACAO_FERRAMENTAS',company:data.company,responsible:data.responsible,provider:providerRows?.[0]||{},commercialParameters:parameterRows?.[0]||{},resources:proposal.snapshot_final?.itens_comerciais||proposal.itens_comerciais||[],scope:proposal.snapshot_final?.resolucoes_escopo||proposal.resolucoes_escopo||[],financial:financialPayload}),updated_at:now},contract=tools.contract?{...tools.contract,...contractPayload}:(await rest('contratos_growth',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({...contractPayload,created_at:now})}))?.[0];
                if(tools.contract)await rest(`contratos_growth?id=eq.${encodeURIComponent(tools.contract.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(contractPayload)});
                ...
                const mail=await sendEmail({email,name,link:generated.link,existing:isExisting,tools:true}); await audit(empresaId,undefined,'Portal de Ferramentas publicado',`Proposta Comercial V${proposalVersion} publicada no Portal. Convite ${mail.sent?'enviado':'gerado'} para ${email}.`);
            ...
            ]), project = official.project || official.legacyFallback && legacyProject || null;
            const financial = (await loadGrowthFinancial(rest, { companyId: empresaId, projectEvolutionId: project?.id }))?.financial || null, canonical = await canonicalPublicationReadiness(official, financial);
            ...
            if (!diagnostic)
                pending.push('Concluir o Diagnóstico');
            if (!String(plan?.parecer_consultor || diagnostic?.parecer_consultor || diagnostic?.parecer || '').trim())
                pending.push('Preencher o Parecer do Consultor');
            if (canonical.legacy && (!plan || !['Plano Concluído', 'Concluído', 'Plano Liberado ao Cliente'].includes(plan.status)))
                pending.push('Concluir o Plano Estratégico');
            if (canonical.legacy) {
                if (!project)
                    pending.push('Preparar um Projeto de Evolução em Rascunho');
                if (project && !canonical.legacyState?.configuration)
                    pending.push('Concluir as Configurações da Implantação');
                if (project && !canonical.legacyState?.executor)
                    pending.push('Definir a Estratégia de Execução e o executor de cada solução recomendada');
                if (project && !project.checklist?.estrategia_marketing_definida)
                    pending.push('Definir a Estratégia de Marketing');
                if (project && !project.checklist?.investimento_aprovado_registrado)
                    pending.push('Registrar o investimento aprovado para esta fase');
                if (project && !canonical.legacyState?.financial)
                    pending.push('Atualizar o Resumo Financeiro');
            }
            else {
                if (!official.publishedPlan)
                    pending.push('Plano Estratégico publicado não encontrado.');
                if (official.publishedPlan && !project)
                    pending.push('Projeto de Evolução não corresponde ao Plano Estratégico vigente.');
                if (official.publishedPlan && !official.implementation)
                    pending.push('Implantação oficial da versão publicada não encontrada.');
                if (official.implementation && !canonical.implementation.hasItems)
                    pending.push('A Implantação oficial não possui ações operacionais.');
                if (canonical.implementation.missingResponsible.length)
                    pending.push('Existem ações de implantação sem responsável.');
                if (canonical.implementation.missingDue.length)
                    pending.push('Existem ações de implantação sem prazo.');
                if (project && !canonical.commercial?.ready)
                    pending.push('Consolidação Comercial 3.0 ainda possui divergências.');
            }
            if (!documentReady)
                pending.push('Prepare o Contrato/Termo para continuar.');
            if (!termSelected && contract && contract.status !== 'Revisado')
                pending.push('Revisar e confirmar o Contrato/Termo');
            for (const item of missingContractualFields(data.company, data.responsible))
                pending.push(`Completar ${item.label}`);
            if (!financial)
                pending.push('Salvar o Financeiro');
            if (financial?.valor_implantacao == null)
                pending.push('Definir o valor da implantação');
            if (!Number(financial?.prazo_contratual || 0))
                pending.push('Definir o prazo contratual');
            if (!Number(financial?.validade_proposta || 0))
                pending.push('Definir a validade da proposta');
            if (project?.exige_pagamento && !String(financial?.link_pix || financial?.link_cartao || financial?.link_assinatura || '').trim())
                pending.push('Informar ao menos um link de pagamento');
            if (pending.length)
                return Response.json({ error: 'Existem pendências antes da publicação.', pending }, { status: 409 });
            ...
            if (intelligentPendencies?.length && !body.confirm_pendencies)
                return Response.json({ error: marketingPending ? 'Existem recomendações de Marketing sem parâmetros definidos.' : 'Atenção. Ainda existem configurações recomendadas que deverão ser concluídas durante a implantação.', pending: intelligentPendencies.map((item: any) => item.titulo), code: 'INTELLIGENT_PENDENCIES' }, { status: 409 });
            ...
            await saveGrowthFinancial(rest, { companyId: empresaId, projectEvolutionId: project.id, payload: { status: 'Portal publicado', publicada_em: now, publicada_por: String(body.usuario || 'Usuário Master'), versao_publicada: version, snapshot_publicado: snapshot, updated_at: now } });
            const mail = await sendEmail({ email, name, link: generated.link, existing: isExisting });
            if (ctx.diagnosticoId)
        ...
        if (['create_release', 'release_existing'].includes(action) && ctx.diagnosticoId) {
            await updatePlanJourney(ctx.diagnosticoId, 'release');
            await audit(empresaId, ctx.diagnosticoId, 'Proposta liberada', 'Conteúdo liberado ao cliente pelo Usuário Master.');
        }
        ...
        if (detail.includes('financeiro_growth_status_check'))
            return Response.json({ error: 'O banco ainda utiliza os status antigos do Financeiro. Execute a migration V35 no Supabase e tente novamente.' }, { status: 409 });
        if (detail.includes('projetos_evolucao_status_check'))
            return Response.json({ error: 'O banco ainda utiliza os status antigos dos Projetos de Evolução. Execute a migration de governança dos projetos no Supabase e tente publicar novamente.' }, { status: 409 });
        return Response.json({ error: detail || 'Não foi possível concluir a Publicação.' }, { status: 500 });
    }
}
export async function PATCH(req: Request) { try {
    const body = await req.json(), token = String(body.access_token || '');
    if (!token || !SUPABASE_URL || !ANON)
    ...
    if (!u.ok)
        return Response.json({ error: 'Convite inválido ou expirado.' }, { status: 401 });
    const user = await u.json();
    const invitation = (await rest(`portal_usuarios?email=eq.${encodeURIComponent(String(user.email).toLowerCase())}&select=convite_expira_em,primeiro_acesso_em&limit=1`))?.[0];
    if (invitation?.convite_expira_em && new Date(invitation.convite_expira_em) < new Date())
        return Response.json({ error: 'Este convite expirou. Solicite um novo convite à Escala Vendas.' }, { status: 410 });
    if (!body.password || String(body.password).length < 8)
        return Response.json({ error: 'A senha deve possuir pelo menos 8 caracteres.' }, { status: 400 });
    const changed = await fetch(`${SUPABASE_URL}/auth/v1/user`, { method: 'PUT', headers: { apikey: ANON, Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ password: body.password }) });
    if (!changed.ok)
        return Response.json({ error: 'Senha não aceita. Por favor, digite outra senha.' }, { status: 400 });
    const now = new Date().toISOString();
    await rest(`portal_usuarios?email=eq.${encodeURIComponent(String(user.email).toLowerCase())}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ auth_user_id: user.id, status_acesso: 'Acesso ativado', ativo: true, acesso_ativado_em: now, primeiro_acesso_em: invitation?.primeiro_acesso_em || now, ultimo_acesso_em: now, convite_link: null, updated_at: now }) });
    const profile = (await rest(`portal_usuarios?email=eq.${encodeURIComponent(String(user.email).toLowerCase())}&select=empresa_id,nome&limit=1`))?.[0];
    if (profile?.empresa_id) {
        const isToolCustomer=Boolean((await rest(`projetos_implantacao_ferramentas?empresa_id=eq.${encodeURIComponent(profile.empresa_id)}&select=id&limit=1`).catch(()=>[]))?.length);
        await sendWelcomeEmail({ email: String(user.email).toLowerCase(), name: profile.nome || 'Cliente', tools:isToolCustomer }).catch(() => { });
        await audit(profile.empresa_id, undefined, invitation?.primeiro_acesso_em ? 'Acesso realizado' : 'Primeiro acesso realizado', invitation?.primeiro_acesso_em ? 'O cliente acessou novamente e atualizou sua senha.' : 'O cliente definiu sua senha e ativou o acesso.');
        const financialContext = await loadGrowthFinancial(rest, { companyId: profile.empresa_id });
        const financial = financialContext?.financial;
        if (financial?.status === 'Portal publicado')
            await saveGrowthFinancial(rest, { companyId: profile.empresa_id, formalizationId: financial.formalizacao_id, payload: { status: 'Cliente acessou', updated_at: now } });
    }
    return Response.json({ ok: true });
}
catch (e: any) {
    return Response.json({ error: e?.message || 'Não foi possível ativar o acesso.' }, { status: 500 });
} }
