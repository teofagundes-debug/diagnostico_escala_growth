...
export async function PATCH(req:Request){
 ...
 const diagnosisValidation=meetingValidation.diagnostico||{},decisionsData=meetingValidation.decisoes||{},decisions=Array.isArray(decisionsData.selecionadas)?decisionsData.selecionadas:[],decisionMap:any={'Implantar CRM':'CRM Comercial','Automatizar WhatsApp':'WhatsApp Oficial','Implantar Google Ads':'Gestão Google Ads','Implantar Meta Ads':'Gestão Meta Ads','Dashboard Executivo':'Dashboard Executivo','Landing Page':'Landing Page','Agente de IA':'Agente de IA','Integrações':'Integrações','Campanhas':'Campanhas'};
 const approved=decisions.filter((name:string)=>name!=='Outro').map((name:string)=>decisionMap[name]||name),removed:string[]=[],added=decisions.includes('Outro')?lines(decisionsData.outro):[],existingResources=Array.isArray(currentImplementation?.recursos)?currentImplementation.recursos:[];
 ...
 for(const name of [...approved,...added])if(!resources.some((x:any)=>x.nome?.toLowerCase()===name.toLowerCase()))resources.push({nome:name,status:'Recomendado',descricao:'Solução aprovada durante a Reunião Estratégica.',tipo:'Sob Demanda'});
 ...
 const validatedSituation=[conclusion.resumo_executivo,meetingValidation.realidade_atual&&`Realidade atual:\n${meetingValidation.realidade_atual}`,conclusion.desafio&&`Principal desafio:\n${conclusion.desafio}`,diagnosisValidation.resposta&&`Validação do diagnóstico: ${diagnosisValidation.resposta}.`,diagnosisValidation.observacoes,diagnosisValidation.informacoes_novas&&`Novas informações:\n${diagnosisValidation.informacoes_novas}`,conclusion.restricoes&&`Restrições identificadas:\n${conclusion.restricoes}`].filter(Boolean).join('\n\n');
 const validatedPriorities=[`Prioridade para os próximos 90 dias:\n${conclusion.prioridade_90_dias}`,conclusion.oportunidade_principal&&`Principal oportunidade:\n${conclusion.oportunidade_principal}`,meetingValidation.oportunidades&&`Oportunidades identificadas:\n${meetingValidation.oportunidades}`].filter(Boolean).join('\n\n');
 ...
 await Promise.all([
  ...
  api(`planos_estrategicos?diagnostico_id=eq.${body.diagnostico_id}`,{method:'PATCH',body:JSON.stringify({status:'Em Consolidação',resumo:conclusion.resumo_executivo,situacao_atual:validatedSituation,objetivos:conclusion.prioridade_90_dias,prioridades:validatedPriorities,proximos_passos:[...approved,...added].join('\n'),parecer_consultor:body.parecer_consultor||null,updated_at:now})}),
