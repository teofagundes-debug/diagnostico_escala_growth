...
export async function PATCH(req:Request){
 ...
 const resources=existingResources.map((item:any)=>approved.some((x:string)=>item.nome?.toLowerCase()===x.toLowerCase())?{nome:item.nome,status:'Recomendado',solucao_aprovada:true,origem:'Diagnóstico → Validação em reunião'}:{nome:item.nome,status:'Não recomendado',solucao_aprovada:false});
 ...
 const library=await api('catalogo_recursos?ativo=eq.true&select=*').then(r=>r.ok?r.json():[]),normalize=(value:string)=>String(value||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]/g,'');
 const traceableRecommendations=[...approved,...added].map((name:string)=>{const solution=library.find((item:any)=>normalize(item.nome)===normalize(name))||library.find((item:any)=>normalize(item.nome).includes(normalize(name))||normalize(name).includes(normalize(item.nome))),benefits=Array.isArray(solution?.beneficios)?solution.beneficios:lines(solution?.beneficios);return[`Diagnóstico → Validação em reunião → Biblioteca de Soluções → ${name}`,solution?.descricao&&`Descrição: ${solution.descricao}`,benefits.length&&`Benefícios: ${benefits.join('; ')}`,solution?.objetivo_padrao&&`Objetivo: ${solution.objetivo_padrao}`].filter(Boolean).join('\n')});
