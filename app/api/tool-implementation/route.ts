...
export async function PATCH(req:Request){try{
 ...
 const commercial=await commercialData(),requested=(body.itens_comerciais||[]).map((item:any)=>({resource_id:item.resource_id,quantity:item.quantidade,unit:item.unidade,origins:item.origens||['CONSULTOR']})),items=priceToolResources(requested,commercial.catalog,commercial.parameters.valor_ui,stored.itens_comerciais||[],body.action==='refresh_prices'),totals=toolCommercialTotals(items),project=(await db(`projetos_implantacao_ferramentas?id=eq.${encodeURIComponent(projectId)}&select=solucoes_selecionadas&limit=1`))[0],resolutions=Array.isArray(body.resolucoes_escopo)?body.resolucoes_escopo:[],scopeValidation=validateToolScopeResolutions(project?.solucoes_selecionadas||[],resolutions,items);
 if(status==='VALIDADA'&&!scopeValidation.valid)return Response.json({error:'Resolva completamente todos os escopos antes de concluir a validação.',details:scopeValidation.errors},{status:409});
 const now=new Date().toISOString(),snapshot={...toolCommercialSnapshot(items,{...commercial.parameters,valor_ui:items[0]?.valor_ui??commercial.parameters.valor_ui}),scope_resolutions:resolutions},financeiro={...body.financeiro,...totals},proposal={sintese:body.sintese||null,configuracao:body.configuracao||{},itens_implantacao:body.itens_implantacao||[],itens_comerciais:items,resolucoes_escopo:resolutions,snapshot_comercial:snapshot,financeiro,observacoes_internas:body.observacoes_internas||null,status,updated_at:now,...(status==='VALIDADA'?{validada_em:now,validada_por:'Usuário Master',snapshot_final:{sintese:body.sintese||null,configuracao:body.configuracao||{},itens_implantacao:body.itens_implantacao||[],itens_comerciais:items,resolucoes_escopo:resolutions,snapshot_comercial:snapshot,financeiro}}:{})};
